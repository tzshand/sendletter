...
> = {
  ...
  legal: {
    ...
    label: "Legal",
    labelFr: "Légal",
    desc: "8.5×14\", brown envelope",
    descFr: "8,5×14\", enveloppe kraft",
  },
  large: {
    maxPages: 15,
    priceAdd: 1.5,
    label: "8.5 × 11",
    labelFr: "8,5 × 11",
    desc: "Full size, brown envelope",
    descFr: "Pleine taille, enveloppe kraft",
  },
};
...
const ICONS: Record<LetterSize, React.ReactNode> = {
  ...
  large: <Package className="w-4 h-4" />,
};
        ...
        return (
          <button
            ...
            className={`relative flex flex-col items-center gap-1 px-3 py-3 rounded-xl border-2 transition-all text-center ${
              active
                ? "border-teal bg-teal/5 text-gray-900"
                : "border-gray-200 hover:border-gray-300 text-gray-500"
            }`}
          >
            <div className={active ? "text-teal" : "text-gray-400"}>
              {ICONS[size]}
            </div>
            ...
            <span className="text-[10px] font-medium mt-0.5">
              {cfg.priceAdd > 0 ? (
                ...
                <span className="text-teal">${BASE_PRICE.toFixed(2)}</span>
              )}
            </span>
            {cfg.priceAdd > 0 && (
